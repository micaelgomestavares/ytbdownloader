...
const DownloadItem: React.FC<DownloadItemProps> = ({ download, onRemove }) => {
  const getStatusConfig = (status: Download['status']) => {
    switch (status) {
      case 'pending':
        return { 
          icon: Hourglass, 
  ...
  return (
    <Card className={`w-full ${download.status === 'error' ? 'border-destructive' : ''}`}>
            ...
            <div className={`mt-1 ${statusConfig.className}`}>
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                ...
                {download.duration && (
                  ...
                  <Badge 
                  variant={download.status === 'completed' ? 'default' : statusConfig.variant} 
                  className={`text-xs ${download.status === 'completed' ? 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900 dark:text-green-100' : ''}`}
                >
                  {download.status === 'completed' ? 'Baixada' : statusConfig.text}
                </Badge>
              </div>
            </div>
          </div>
      ...
      <CardContent className="pt-0">        {download.status === 'downloading' && (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              {download.speed && (
                <span>Velocidade: {download.speed}</span>
              )}
              {download.eta && (
                <span>Tempo restante: {download.eta}</span>
              )}
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-2 p-3 bg-green-50 rounded-md border border-green-200 dark:bg-green-950 dark:border-green-800">
            ...
            <p className="text-sm text-green-700 dark:text-green-300">
              ...
              {Array.isArray(download.outputFile) && (
                <span className="ml-1">
                  - {download.outputFile.filter(r => r.success).length} arquivos
                </span>
              )}
            </p>
          </div>
